...
When `PUBLIC_CHAT_EXTERNAL_SOURCES_ENABLED=true`, Jev may select up to three
approved source adapters. Requests are live on demand and the selected adapters
run in parallel with bounded results and an approximately 8.5-second server
deadline. The verified live adapters currently cover the BetterGov.PH Budget API
and Philippine Data Explorer Statistics API. The registry also records Juris,
BatasWatch, BetterGov services and officials, classification codes, flood
control, DPWH public works, and ASEAN indicators; each remains unavailable until
its read-only machine contract is verified. BetterLal-lo never scrapes those
sites or lets Jev create arbitrary URLs, endpoints, headers, or API arguments.

Jev checks the returned evidence for relevance, sufficiency, and conflicts.
Direct evidence links are generated by code. Jev may select a related link only
from a bounded candidate list, and BetterLal-lo validates the final URL against
the source allowlist before displaying it.
