# Public information sources

BetterLal-lo uses a source-first workflow. A page should tell readers what it is based on, and an update date should be added whenever a contributor checks a record. Published records are repository-managed snapshots, not a live municipal database.

## Primary sources

- Municipality of Lal-lo: municipal notices, office publications, ordinances, resolutions, budgets, projects, and official contact channels.
- Provincial Government of Cagayan: municipal directory, provincial notices, and linked public services.
- Philippine Statistics Authority: geographic and barangay classification data.
- Commission on Audit: audit reports and financial records for local government units.
- Department of Budget and Management: national budget and local-government open-data sources.
- Cagayan PDRRMO: disaster-risk-reduction and emergency information relevant to Lal-lo.

## Scope and verification rules

- `Verified` means the specific claim on the page matches the cited official source.
- Municipal, provincial, regional, and national authorities are labeled separately. A higher-level source may publish a Lal-lo-specific record, but a general agency homepage cannot verify a Lal-lo fee, requirement, contact, or project.
- Historical and preliminary records may be published when their source period or qualification is visible. They are not presented as current or final.
- Search results, third-party directories, and social posts can identify leads but are not direct evidence for publication.
- Conflicting official names or values stay pending until the source dates and jurisdiction are reconciled.
- COA records for the Lal-lo Water District are not municipal-government audit records and are excluded from municipal finance summaries.

Import candidates are kept in the repository source ledger and draft folder for human review. Draft imports never publish automatically; a reviewer must confirm jurisdiction, dates, values, and wording before a factual record moves into the app data.

## Verification rule

When sources conflict, the page should show the conflict or defer to the most recent official publication. A search result, social-media post, or old directory is not enough on its own for a current phone number, officeholder, fee, deadline, or emergency instruction.

## Useful links

- [PSA Lal-lo profile](https://psa.gov.ph/classification/psgc/barangays/0201516000)
- [Cagayan municipal directory](https://cagayan.gov.ph/city-and-municipalities/)
- [DBM Open Data](https://www.dbm.gov.ph/index.php/open-data)
- [COA](https://www.coa.gov.ph/)
- [Cagayan PDRRMO](https://pdrrmo.cagayan.gov.ph/)
- [PSA OpenSTAT 2024 Population tables](https://openstat.psa.gov.ph/PXWeb/pxweb/en/DB/DB__1A__PO_2024/?tablelist=true)
- [PhilGEPS Lal-lo notice 13112765](https://notices.philgeps.gov.ph/GEPSNONPILOT/Tender/PrintableBidNoticeAbstractUI.aspx?refid=13112765)
- [NHCP Lalloc-Nueva Segovia registry entry](https://philhistoricsites.nhcp.gov.ph/registry_database/lalloc-nueva-segovia/)

Checked: 21 September 2026
